...
// ---------------------------------------------------------------------------------------------------------------------
// Insert nPart particles from source_id at dest_id in dest_parts
// ---------------------------------------------------------------------------------------------------------------------
void Particles::cp_particles(int source_id, int nPart, Particles &dest_parts, int dest_id )
{
    for (unsigned int i=0; i<Position.size(); i++) {
        dest_parts.Position[i].insert( dest_parts.Position[i].begin() + dest_id, Position[i].begin() + source_id, Position[i].begin() + source_id + nPart );
        dest_parts.Position_old[i].insert( dest_parts.Position_old[i].begin() + dest_id, Position_old[i].begin() + source_id, Position_old[i].begin() + source_id + nPart );
    }
    ...
    if (isTestParticles)
        dest_parts.Id.insert( dest_parts.Id.begin() + dest_id, Id.begin() + source_id, Id.begin() + source_id + nPart );
...
// ---------------------------------------------------------------------------------------------------------------------
// Move particle part1 into part2 memory location, for 1D and 2D.
// ---------------------------------------------------------------------------------------------------------------------
void Particles::overwrite_part(int part1, int part2)
{
    for(int iDim=0; iDim<Position.size(); iDim++)
    {
        Position    [iDim][part2] = Position    [iDim][part1];
        Position_old[iDim][part2] = Position_old[iDim][part1];
    }
...
void Particles::overwrite_part(int part1, int part2, int N)
{
    for (unsigned int j=0; j< (unsigned int) N; j++) {
        for(int iDim=0; iDim<Position.size(); iDim++)
        {
            Position    [iDim][part2+j] =  Position    [iDim][part1+j];
            Position_old[iDim][part2+j] =  Position_old[iDim][part1+j];
        }
