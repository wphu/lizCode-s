...
public:
	...
	static vector<PSI*> create(PicParams& params, InputData &ifile, vector<Species*>& vecSpecies)
	{
	    vector<PSI*> vecPSI;
	    ...
	    for (unsigned int n_PSI = 0; n_PSI < numPSI; n_PSI++) 
		{
			...
			if(params.geometry == "1d3v" && PSI_type == "sputtering")
			{
		        ...
		        vecPSI.push_back( new PSI1D_Sputtering(params, vecSpecies, sgroup1[0],sgroup2[0],is_self_consistent, psiPos, emitTemp) );
			...
			else if(params.geometry == "1d3v" && PSI_type == "RefDep")
			{
		        ...
		        vecPSI.push_back( new PSI1D_RefDep(params, vecSpecies, sgroup1[0],sgroup2[0], is_self_consistent, psiPos, emitTemp) );
			}
			...
			else if(params.geometry == "1d3v" && PSI_type == "Recycling")
			{
		        ...
		        vecPSI.push_back( new PSI1D_Recycling(params,sgroup1[0], sgroup2[0], is_self_consistent, psiPos, emitTemp, recycling_factor) );
			}
			...
			else if(params.geometry == "1d3v" && PSI_type == "SEE")
			{
		        ...
		        vecPSI.push_back( new PSI1D_SEE(params,sgroup1[0],sgroup2[0], is_self_consistent, psiPos, emitTemp, SEEYield) );
